...
import { getUserKitForShareQuery } from '@services/graphql/queries/userKit';
...
const UserSkillSharePage: NextPageWithLayout = ({
	...
	useEffect(() => {
		console.log('start', hashLink, userKitData);
	...
	const experience = (): number => {
		if (userKitData.experience) {
			return userKitData.experience.years * 12 + userKitData.experience.months;
		}

		return 0;
	};

	const handleUserSkillClick = (item) => {
		if (!item.shareLink) {
	...
	return (
		<>
			<Head>
				<title>I&apos;m {userKitData?.profession.name}</title>
				...
				<meta property="og:title" content={`I&apos;m ${userKitData?.profession.name}`} key="og:title" />
				<meta
				...
				<meta name="twitter:title" content={`I&apos;m ${userKitData?.profession.name}`} />
				...
				{userKitData.viewMode !== UserSkillViewModeEnum.EVERYONE && (
					<>
			...
			<div className={styles.container}>
					...
					<Col xs={{ span: 24, order: 2 }} md={{ span: 18, order: 1 }}>
						<Space direction="vertical" size={40} style={{ width: '100%' }}>
							...
							<Space direction="vertical" className={styles.titleSection}>
								<h1>
									<div>I am</div>
									<div className={styles.title}>{capitalizedText(userKitData?.profession.name)}</div>
								</h1>
							</Space>

							{userKitData?.description?.length > 0 && (
								<div className={styles.descriptionSection}>
									<p className={styles.description}>{userKitData?.description ? readyText(userKitData?.description) : ''}</p>
								</div>
							)}

							{userKitData?.userSkills?.length > 0 && (
								<div className={styles.userSkillsSection}>
									<div className={styles.headerContainer}>
										<h2>I have the following skills</h2>
									</div>
									<List
										size="small"
										itemLayout="horizontal"
										dataSource={userKitData?.userSkills.sort((a, b) => {
										...
										renderItem={(item: any) => {
											...
											return (
													...
													actions={[
														<>
															{item.viewMode !== UserSkillViewModeEnum.ONLY_ME ? (
																<Button type="text" onClick={() => handleUserSkillClick(item)}>
																	Details <TbArrowNarrowRight />
																</Button>
															) : null}
														</>,
													]}
												>
													...
													/>
													{screens.md && <Typography.Text type="secondary">{experienceAsText(item.experience)}</Typography.Text>}
												</List.Item>
											);
										}}
									/>
							...
							{userKitData?.userTools?.length > 0 && (
								<div className={styles.toolsSection}>
									<div className={styles.headerContainer}>
										<h2>I am using for this</h2>
									</div>
									<Space className={styles.list} size="middle">
										{userKitData?.userTools.map((item: any, idx: number) =>
											item.description ? (
												<Badge
													count={
														<Popover
													...
													key={idx}
												>
													<div className={styles.listItem}>{item.workTool.name}</div>
												</Badge>
											) : (
							...
							{userKitData?.userJobs?.length > 0 && (
								<div className={styles.jobsSection}>
									<div className={styles.headerContainer}>
										<h2>I used these skills in the following jobs</h2>
									</div>
									{userKitData?.userJobs?.length > 0 && (
										<Timeline mode="left">
											{userKitData?.userJobs.map((item: any, indx: number) => (
												<Timeline.Item key={10 + indx}>
                              ...
                              {moment(item.startedAt).format('MMM, YYYY') +
                                ...
                                (item.finishedAt ? moment(item.finishedAt).format('MMM, YYYY') : 'Now')}
															...
															{item.userJobs.map((userJob, idx: number) => (
																<div key={idx}>
																	{!!userJob && (
							...
							{userKitData?.userSchools?.length > 0 && (
								<div className={styles.schoolsSection}>
									...
									{userKitData?.userSchools?.length > 0 && (
										<Timeline mode="left">
											{userKitData?.userSchools.map((item: any, indx: number) => (
												<Timeline.Item key={20 + indx}>
				...
				{userKitData?.userFiles?.length > 0 && (
					<Row style={{ marginTop: '40px' }}>
						<Col flex={1}>
							<div className={styles.worksSection}>
								...
								<FileGallery
									fileList={userKitData.userFiles}
									onlyView
				...
				<ReactViewer
					...
					activeIndex={userKitData?.userFiles
...
export async function getServerSideProps(context) {
	try {
		...
		if (hashLink.length > 0) {
			...
			props = data?.userKitForShare;

			if (props.viewer !== 'me') {
				props.userSkills = props.userSkills.filter(
					(userSkill: any) => userSkill.viewMode !== UserSkillViewModeEnum.ONLY_ME,
				);
			}
		}
